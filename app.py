...
def main():
    ...
    # App header
    ####################################################################
    st.markdown("<h1 class='main-title'>Agno Agentic RAG </h1>", unsafe_allow_html=True)
    st.markdown(
        "<p class='subtitle'>Seu assistente de pesquisa inteligente com tecnologia Agno</p>",
    ...
    if prompt := st.chat_input("👋 Pergunte algo sobre o Agno!"):
    ...
    # Adicionar logo do Agno
    st.sidebar.image("imgs/agnologo.png", width=200)
    ...
    if (
        ...
        if input_url not in st.session_state.loaded_urls:
            alert = st.sidebar.info("Processando URLs...", icon="ℹ️")
            if input_url.lower().endswith(".pdf"):
                try:
                    ...
                    st.sidebar.error(f"Erro ao processar PDF: {str(e)}")
                    docs = []
            else:
            ...
            if docs:
                ...
                st.sidebar.error("Não foi possível processar a URL fornecida")
            alert.empty()
        else:
            st.sidebar.info("URL já carregada na base de conhecimento")
    ...
    # Sample Question
    ###############################################################
    st.sidebar.markdown("#### ❓ Perguntas Exemplares")
    if st.sidebar.button("📝 Resumir"):
        add_message(
            "user",
            "Você pode resumir o que está atualmente na base de conhecimento (use a ferramenta `search_knowledge_base`)?",
        )
    ...
    with col1:
        if st.sidebar.button(
            "🔄 Novo Chat", use_container_width=True
